...
public class GameCLI {
    ...
    public GameCLI(Tank bot1, Tank bot2) throws Exception {
    ...
    public int run() {
        ...
        if (turn >= MAX_NUMBER_OF_TURNS) {
            System.out.println("[Game] Max turns reached");
            game_state = -1;
        }
        return game_state;
    }
    ...
    private void document(int turnIndex, int tankId, Action action) {
        ...
        if (logIndex >= 0 && logIndex < m_logs.length) {
            m_logs[logIndex] = new Logs(tankId, action, Board.getLocation(tankId),
                    m_tanks[tankId].get_direction(), m_tanks[tankId].get_turretDirection(),
                    m_tanks[tankId].getHealth());
        }
    }
